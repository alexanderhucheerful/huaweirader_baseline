...
parser.add_argument('-epochs', default=70, type=int, help='sum of epochs')
#LR_step_size = 20000
...
save_dir = '/media/workdir/hujh/hujh-new/huaweirader_baseline/model_parameters/classify_test/unet_model/'
...
train_loaders=  torch.utils.data.DataLoader(train_datasets,batch_size  =args.batch_size,shuffle=True,num_workers = 4,pin_memory=True)

#print('checkpoint of len train_laoders')

valid_loaders =  torch.utils.data.DataLoader(valid_datasets,batch_size  = args.batch_size,shuffle=False,num_workers = 4,pin_memory=True)
...
if os.path.exists(os.path.join(save_dir, 'unetcheckpoint.pth.tar')):
    # load existing model
    print('==> loading existing model')
    model_info = torch.load(os.path.join(save_dir, 'unetcheckpoint.pth.tar'))
...
for epoch in tqdm(range(cur_epoch, args.epochs + 1)):
    ...
    if epoch %2==0 and epoch >0:
        optimizer = adjust_learning_rate(optimizer, args.lr * 0.7)
...
###############################################tenosrboard太麻烦######
            ...
            plt.savefig('/media/workdir/hujh/hujh-new/huaweirader_baseline/log/demolog/unetloss.png')
            plt.close(1)
#################################################################################
            #changepoint
            save_checkpoint({
                ...
                save_dir=save_dir,
                filename='unetcheckpoint.pth.tar')
################# valid ########################################################
        ...
        if True:
            ...
            for headid in range(1):
                ...
                for ind, (_, (inputframes,targetframes)) in enumerate(valid_loaders):
                    ...
                    val_loss = criterionmse(val_output, val_targets)
    ...
    plt.legend()
    #changepoint
    plt.savefig('/media/workdir/hujh/hujh-new/huaweirader_baseline/log/demolog/unetval.png')
    plt.close(2)
