...
class MIMBlock(nn.Module):
	...
	def MIMS(self, x, h_t, c_t): # MIMS
		...
		c_new = f_ * c_t + i_ * torch.tanh(g_) #S

		# oc_weight
		o_c = torch.mul(c_new, self.oc_weight)
		
		h_new = torch.sigmoid(o_ + o_c) * torch.tanh(c_new) #	C

		return h_new, c_new

	def forward(self, x, diff_h, h, c, m,S):
		# 初始化隐藏层 记忆 空间
		...
		return new_h, new_c, new_m,S # 大小均为 [batch, in_height, in_width, num_hidden]
