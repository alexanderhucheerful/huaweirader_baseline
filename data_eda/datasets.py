...
class 	ClassifyTestloadedDataset(Dataset):
	def __init__(self, root_dir,pd_path,npy_path,frame_num=None, time_freq = None, ps = None):
		#the pd path is your pkl location  root_dir is your png loaction
		#这两个参数是用来控制滑窗变量，因为一个样本只能切出来一个seq所以不使用滑窗
		self.fnum = frame_num
		...
		npydata = list(npydata)
		self.index = npydata
		self.hardindex = np.load('/media/workdir/hujh/hujh-new/huaweirader_baseline/data_eda/hard_test_demo.npy')
	...
	def __getitem__(self, idx):
		...
		return mark,filename, inputframes
